...
Also thought about what happens if there's a power interruption — if the state machine resets randomly you could end up with two green lights on at the same time which is obviously a problem. The fix would be defaulting to all-red after power restores before resuming normal operation.
